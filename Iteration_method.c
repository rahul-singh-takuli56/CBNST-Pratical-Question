...
float findat(float x){
    return 1/sqrt(1+x);  // value of x in function
}
float differn(float x){
...
int main(){
    ...
    x0=(x1+x2)/2;
    if(differn(x0)>1){
        printf("Function form is not correct Iteratin method can't be applied ");
        return 0;
    }
    while(i<=max_iteration){
        x=findat(x0);
        if(fabs(x-x0)<error){
            printf("Iteration is %d Final root is %f",i,x);
            return 0;
        }
